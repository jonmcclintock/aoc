#!/bin/bash
IN="sayndz zfxlkl attjtww cti sokkmty brx fhh suelqbp
...
SUM=0; IFS=$'\n'; for line in $IN; do IFS=" "; ROW=(); for word in $line ; do ROW+=(`echo $word | grep -o . | sort | tr -d "\n"`); done ; WORDS=`echo ${ROW[@]} | wc -w`; UNIQ=`printf '%s\n' ${ROW[@]} | sort | uniq | wc -w`; if [[ $WORDS -eq $UNIQ ]] ; then SUM=`expr $SUM + 1` ; fi ; done ; echo $SUM
