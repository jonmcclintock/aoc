...
func main() {
	...
	for _, line := range in {
		...
		s2 := strings.Split(nodeDef, " ")
		name, weight := s2[0], s2[1]
		...
		if len(s1) > 1 {
			for _, childName := range strings.Split(s1[1], ", ") {
				if _, ok := index[childName]; !ok {
					index[childName] = &node{
						name: childName,
					}
				}
				index[childName].parent = n
				n.children = append(n.children, index[childName])
			}
		}
	}
	...
	for _, n := range index {
		if n.parent == nil {
			fmt.Printf("Node '%s' has no parent\n", n.name)
			fmt.Printf("Total weight from %s is: %d\n", n.name, getWeight(n))
			checkWeight(n)
		}
	}
}

func getWeight(n *node) int {
	sum := n.weight
...
func checkWeight(n *node) bool {
	...
	// If there aren't at least two children, there's no chance of a mismatch.
	if len(n.children) < 2 {
		return false
	}

	sort.Slice(n.children, func(i, j int) bool {
		return n.children[i].totalWeight < n.children[j].totalWeight
	})

	if n.children[0].totalWeight != n.children[1].totalWeight {
		fmt.Printf("%s (%d) has mismatched child weights\n", n.name, n.weight)
		fmt.Printf("Child %s (%d) has total weight %d, should be %d\n",
			n.children[0].name, n.children[0].weight,
			n.children[0].totalWeight, n.children[1].totalWeight)
		return true
	}

	l := len(n.children)
	if n.children[l-1].totalWeight != n.children[l-2].totalWeight {
		fmt.Printf("%s (%d) has mismatched child weights\n", n.name, n.weight)
		fmt.Printf("Child %s (%d) has total weight %d, should be %d\n",
			n.children[l-1].name, n.children[l-1].weight,
			n.children[l-1].totalWeight, n.children[l-2].totalWeight)
		return true
	}
