...
func findMaxStrength(comps componentList, pinCount int, curPath []component) ([]component, int) {
	...
	for _, c := range comps[pinCount] {
		...
		path, strength := findMaxStrength(comps, testCount, testPath)
...
func findMaxLength(comps componentList, pinCount int, curPath []component) ([]component, int) {
	used := map[component]bool{}
	for _, c := range curPath {
		used[c] = true
	}
	...
	for _, c := range comps[pinCount] {
		...
		testPath[len(curPath)] = c
		path, strength := findMaxLength(comps, testCount, testPath)
		if (len(path) > maxLength) || ((len(path) == maxLength) && (strength > maxStrength)) {
			maxLength = len(path)
			maxStrength = strength
			maxPath = path
		}
...
func main() {
	...
	fmt.Printf("Finding strongest bridge...\n")
	bridge, strength := findMaxStrength(comps, 0, []component{})
	fmt.Printf("Done!\n\n")
	dumpBridge(bridge, strength)

	fmt.Printf("\nFinding longest bridge...\n")
	bridge, strength = findMaxLength(comps, 0, []component{})
	fmt.Printf("Done!\n\n")
	dumpBridge(bridge, strength)
}
